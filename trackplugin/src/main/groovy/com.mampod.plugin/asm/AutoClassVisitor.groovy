...
import com.mampod.plugin.GlobalConfig
import com.mampod.plugin.bean.AutoClassFilter
import com.mampod.plugin.util.AutoMatchUtil
...
public class AutoClassVisitor extends ClassVisitor {
    ...
    void visit(int version, int access, String name, String signature, String superName, String[] interfaces) {
        mClassName = name.contains('$') ? name.substring(0, name.indexOf('$')) : name
//        mClassName = name
    ...
    void visitEnd() {
        if (GlobalConfig.isOpenLogTrack) {
//            MethodVisitor mv
            if (LogAnalyticsUtil.isInstanceOfActivity(mSuperName)) {
                //防止activity没有复写oncreate方法，再次检测添加
                Iterator<Map.Entry<String, LogMethodCell>> iterator = LogHookConfig.sActivityMethods.entrySet().iterator()
                while (iterator.hasNext()) {
                    ...
                    mv.visitEnd()
                    //标记此方法已插入动态代码
                    mv.visitAnnotation("Lcom/mampod/plugin/annotation/AutoDataInstrumented;", false)
                }

            } else if (LogAnalyticsUtil.isInstanceOfFragment(mSuperName)) {
